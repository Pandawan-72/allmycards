const DE = {
  common: {
    later: "Später",
    cancel: "Abbrechen",
    save: "Speichern",
    add: "Hinzufügen",
    delete: "Löschen",
    confirm: "Bestätigen",
    close: "Schließen",
    back: "Zurück",
    yes: "Ja",
    no: "Nein",
    ok: "OK",
    loading: "Laden…",
    edit: "Bearbeiten",
    share: "Teilen",

    phone: "Telefon",
    website: "Webseite",
    disabled: "Deaktiviert",
    scan: "Scannen",
  },
  auth: {
    emailInUse: "Diese E-Mail-Adresse wird bereits verwendet. Melden Sie sich mit Google an oder verwenden Sie „Passwort vergessen“, um ein Passwort festzulegen.",
    forgotPassword: "Passwort vergessen?",
    resetTitle: "Passwort zurücksetzen",
    resetDesc: "Geben Sie Ihre E-Mail-Adresse ein, und wir senden Ihnen einen Link zum Zurücksetzen Ihres Passworts.",
    resetSend: "Link senden",
    resetSuccess: "E-Mail gesendet! Überprüfen Sie Ihr Postfach (auch die Ordner Spam/Werbung).",
    resetError: "E-Mail konnte nicht gesendet werden. Bitte überprüfen Sie die Adresse.",
    welcome: "Willkommen.",
    subtitle: "Alle Ihre Karten, immer dabei.",
    email: "E-Mail",
    password: "Passwort",
    name: "Name",
    signIn: "Anmelden",
    signUp: "Konto erstellen",
    signingIn: "Anmelden…",
    signingUp: "Erstellen…",
    or: "oder",
    continueWithGoogle: "Mit Google fortfahren",
    noAccount: "Noch kein Konto?",
    goSignUp: "Registrieren",
    haveAccount: "Bereits ein Konto?",
    goSignIn: "Anmelden",
    pwdMin: "Das Passwort muss mindestens 6 Zeichen lang sein.",
    fillAll: "Alle Felder sind erforderlich.",
    fillCreds: "Bitte geben Sie Ihre Anmeldedaten ein.",
    googleFailed: "Google-Anmeldung fehlgeschlagen.",
    welcomeName: "Willkommen, {{name}}",
  },
  home: {
    longPressHint: "Lange drücken, um eine Karte zu bearbeiten oder zu löschen",
    trialDaysLeft_one: "Noch {{count}} Tag mit PRO freigeschaltet",
    trialDaysLeft_other: "Noch {{count}} Tage mit PRO freigeschaltet",
    lockedCard: "Pro zum Freischalten",
    title: "Meine Karten",
    empty: "Noch keine Karten. Tippen Sie auf +, um eine hinzuzufügen.",
    search: "Karte suchen…",
    allCards: "Alle Karten",
    upgrade: "Pro werden",

    tapShort: "KURZ AUF DIE KARTE TIPPEN",
    tapShortDesc: "Karte / Barcode anzeigen",
    tapLong: "LANG AUF DIE KARTE TIPPEN",
    tapLongDesc: "Bearbeiten oder löschen",
    addCardTitle: "Karte hinzufügen",
    addCardMsg: "Welche Art von Karte möchten Sie erstellen?",
    addStandardCard: "Standardkarte",
    addVCardSub: "Scanbarer vCard QR-Code",
    addStandardCardSub: "Strichcode, QR-Code, Foto...",
    scanToCreate: "Scannen zum Erstellen",
    scanToCreateSub: "Strichcode oder QR-Code",
    proUnlocked: "PRO-Version aktiviert",
    freeLimit: "Sie haben das Limit von 5 kostenlosen Karten erreicht.",
  },
  card: {
    newCard: "Neue Karte",
    name: "Kartenname",
    editCard: "Karte bearbeiten",
    discardTitle: "Änderungen verwerfen?",
    discardMessage: "Möchten Sie diese Seite verlassen und Ihre aktuellen Änderungen verwerfen?",
    discardConfirm: "Ja, verwerfen",
    discardCancel: "Nein, hier bleiben",
    namePh: "Supermarkt, Bildung & Bibliothek, Fitnessstudio…",
    useLetterLogo: "Buchstaben-Logo",
    markFavorite: "Als Favorit markieren",
    category: "Kategorie",
    color: "Farbe",
    barcodeType: "Code-Typ",
    scanBarcode: "Barcode scannen",
    scanPhoto: "Karte scannen (Foto)",
    takePhoto: "Foto aufnehmen",
    pickFromGallery: "Aus Galerie wählen",
    scanFront: "Vorderseite",
    scanBack: "Rückseite",
    tapToShow: "Tippen zum Anzeigen",
    noBarcode: "Kein Barcode",
    toggleBarcode: "Strichcode",
    rotateHint: "Drehen Sie ins Querformat, um zu vergrößern ↻",
    noBarcodeOrPhoto: "Kein Strichcode oder Foto.",
    frontTitle: "Vorderseite der Karte",
    backTitle: "Rückseite der Karte",
    deleteConfirm: "\"{{name}}\" löschen?",
    shareCard: "Karte teilen",
    createVCard: "vCard erstellen",
    generateVCard: "Mein vCard-QR-Code generieren",
    expiresAt: "Läuft ab am",
    notes: "Notizen",

    protect: "Diese Karte schützen",
    protectSub: "PIN zum Anzeigen anfordern",
    pinRequired: "PIN erforderlich",
    pinRequiredMsg: "Bitte erstellen Sie zuerst Ihre PIN in den Einstellungen.",
    expiryAlert: "✅ Benachrichtigung 30 Tage vor Ablauf",
    alertBefore: "Benachrichtigen",
    alertDays: "Tage vorher",
    notesPh: "Kundennummer, Bemerkungen…",
  },
  vcard: {
    firstName: "Vorname",
    lastName: "Nachname",
    org: "Unternehmen",
    email: "E-Mail",
    phone: "Telefon",
    namePh: "vCard-Name",
    edit: "vCard bearbeiten",
    web: "Webseite",
    errorTitle: "Fehlende Felder",
    errorMsg: "Bitte geben Sie mindestens einen Vor-, Nachnamen oder ein Unternehmen ein.",
    identity: "Identität",
    contact: "Kontakt",
    defaultName: "vCard",
  },

  categories: {
    loyalty: "Treue",
    transport: "Transport",
    sport: "Sport & Freizeit",
    health: "Gesundheit",
    bank: "Bank",
    library: "Bildung & Bibliothek",
    identity: "Identität",
    businesscard: "Visitenkarte",
    other: "Sonstiges",
  },
  settings: {
    biometric: "Biometrische Entsperrung",
    biometricDesc: "Fingerabdruck oder Face ID zusätzlich zum PIN-Code verwenden",
    pin: "PIN-Code",
    pinTitle: "Schützen Sie Ihre Karten",
    pinDesc: "Legen Sie einen 4-stelligen PIN fest, um Ihre sensiblen Karten zu sichern.",
    pinSet: "PIN-Code festlegen",
    pinDisable: "PIN-Code deaktivieren",
    pinNew: "Neuer PIN-Code",
    pinConfirm: "PIN-Code bestätigen",
    pinEnter: "Geben Sie einen 4-stelligen Code ein",
    pinRepeat: "Geben Sie denselben Code erneut ein",
    pinWrong: "Falscher Code",
    pinMismatch: "Codes stimmen nicht überein",
    pinChange: "PIN-Code ändern",
    pinCurrent: "Aktuellen PIN eingeben",
    pinEnabled: "Aktiviert 🔒",
    pinDisabled: "Deaktiviert",
    account: "Konto",
    preferences: "Einstellungen",
    proPlan: "Pro-Plan",
    proActive: "Pro-Zugang aktiv", proInactive: "Pro-Zugang inaktiv", googlePlayAccount: "Verknüpftes Google Play-Konto",
    logout: "Abmelden",
    version: "Version",
    language: "Sprache",
    darkMode: "Dunkelmodus",
    title: "Einstellungen",
    backup: {
      section: "SICHERUNG",
      exportTitle: "Karten exportieren",
      exportSub: "In Datei speichern",
      importTitle: "Sicherung wiederherstellen",
      importSub: "Aus Datei importieren",
      importConfirmTitle: "Sicherung wiederherstellen",
      importConfirmMsg: "Dadurch werden alle Ihre aktuellen Karten ersetzt. Fortfahren?",
      importSuccess: "Ihre Karten wurden erfolgreich wiederhergestellt.",
      importSuccessTitle: "Sicherung wiederhergestellt",
      proRequired: "Pro erforderlich",
    },
  },
  paywall: {
    title: "Pro werden",
    subtitle: "Alle Funktionen freischalten",
    free: "Kostenlos",
    oneTime: "Einmalige Zahlung",
    trialEndingTitle: "Ihre Pro-Testphase endet bald",
    trialEndingBody: "Ab der 6. Karte werden zusätzliche Karten gesperrt, wenn Sie nicht auf Pro upgraden.",
    goProNow: "Jetzt Pro werden",
    feature1: "Unbegrenzte Karten",
    feature2: "PIN-Schutz",
    feature3: "Startbildschirm-Widget",
    feature4: "Ablaufbenachrichtigungen",
    feature5: "Sicherung & Wiederherstellung",
    feature6: "Kategoriefilter",
    feature7: "Karte teilen",
    freeCards: "5 Karten",
    feature8: "vCard-Visitenkarte",
    feature9: "Dunkelmodus",
    feature10: "Favoriten-Karten",
    buyLifetime: "Kaufen — 3,99 €",
    restore: "Käufe wiederherstellen",
    once: "Einmaliger Kauf, lebenslanger Zugang",
    alreadyPro: "Sie sind bereits Pro!",
    trialBanner: "Kostenlose Testversion — noch {{hours}}h ✨",
    trialExpired: "Testversion beendet — Werden Sie Pro",
  },
  legal: {
    aboutSection: "ÜBER",
    privacyTitle: "Datenschutzrichtlinie",
    termsTitle: "Nutzungsbedingungen",
  },
  langs: {
    fr: "Französisch",
    en: "Englisch",
    de: "Deutsch",
    es: "Spanisch",
    it: "Italienisch",
    pt: "Portugiesisch",
    nl: "Niederländisch",
    ru: "Russisch",
  },
};

export default DE;